...
6. Detects archived repositories: updates data/archived_repos.json (url, name, date, notes) as
   ...
   workflow commits data/archived_repos.json when it changes.
...
ARCHIVED_REPOS_LIST_PATH = "data/archived_repos.json"  # Persistent list (committed); retired/keeping lookup
...
def load_archived_repos_list() -> Tuple[List[Dict[str, Any]], set]:
    """
    Load the persistent archived-repos list from data/archived_repos.json if it exists.
...
def save_archived_repos_list(existing_entries: List[Dict[str, Any]], new_entries: List[Dict[str, Any]]):
    """
    Merge new entries into the persistent list (by url) and write to data/archived_repos.json.
